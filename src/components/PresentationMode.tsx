...
import { WorkflowData } from './WorkflowEditor';
import '../styles/PresentationMode.css';
import { SUPPORTED_FONTS, FontFamily } from '../constants/fonts';
import { SPEObject } from '../types/spline';

// Import types from spline
...
const PresentationMode: React.FC<Props> = ({ workflow }) => {
  ...
  const [fontFamily, setFontFamily] = useState<FontFamily>(SUPPORTED_FONTS.arial);
  ...
  const { setFontFamily: themeSetFontFamily } = useTheme();
  ...
  const availableTitles = workflow.nodes.map(node => node.data.label);
    ...
    >
      ...
      <div className="navigation-controls">
        <button
          ...
          disabled={!workflow.nodes.find((node, index) => node.id === currentNodeId && index > 0)}
        >
          Previous
        </button>
        <button
          onClick={handleNextSlide}
          disabled={!workflow.nodes.find((node, index) => node.id === currentNodeId && index < workflow.nodes.length - 1)}
        >
          Next
        </button>
        <VoiceControlButton
      ...
      {isListening && (
        <div className="voice-feedback">
          <p>Voice Control: {isListening ? 'Listening...' : 'Paused'}</p>
          {error && <p className="error">Error: {error}</p>}
          {!isSupported && <p className="error">Speech recognition is not supported in this browser</p>}
        </div>
      )}
    </div>
  );
